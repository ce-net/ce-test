...
//!
//! - `ce-test list [--config cetest.toml]`                        — the configured suites.
//! - `ce-test run  [--config …] [--suite NAME] [--tier T3] [--on fleet=mine]` — run them + report.
...
fn main() -> Result<()> {
    ...
    match cmd {
        ...
        _ => {
            eprintln!("usage: ce-test [run|list] [--config cetest.toml] [--suite NAME] [--tier T1|T2|T3] [--on <target>]");
            Ok(())
        }
    }
}
